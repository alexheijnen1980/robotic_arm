...
with open(filename, 'w') as csvfile:
    ...
    msg = bus.recv()
    shaft_angle = 2 * np.pi * int.from_bytes(msg.data[:2], byteorder = 'big') / (rotor_counts_per_rev * reduction)
...
# Determine current shaft angle in radians
        ...
        print(f"target: {target:.2f}, shaft_angle: {shaft_angle:.2f} rad")

# Determine current speed in radians per second
        rotor_speed =  int.from_bytes(msg.data[2:4], byteorder = 'big')
        if rotor_speed > 100:
            rotor_speed = 0
        speed = 2 * np.pi * rotor_speed / (60 * reduction)
        print(f"speed: {speed} rad/s")

# Determine current using controller and convert to hex
        current =  Kp * (target - shaft_angle) - Kd * speed
